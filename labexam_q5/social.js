...
function findForce(a){
    ...
    if(a.type==0){
        ...
        fyatt += 10*(100+timeout - a.q[1])
        fyrep += 1*((a.q[1]-100)/(d*d))
    }else{
        let d = distance(a.q,[a.q[0],400])
        fyatt += 10*(400-timeout - a.q[1])
        fyrep += 1*((a.q[1]-400)/(d*d)) 
    }

    // force from gate
    if(a.type==0){
        let d = distance(a.q,[250,gateuy])
        fyrep += 100000*((a.q[1]-gateuy)/(d*d))

        if(a.q[1]<gateuy || a.q[1]>gatedy){
            fxrep += 800000*((a.q[0]-250)/(d*d)) 
        }

    }else{
        let d = distance(a.q,[250,gatedy])
        fyrep += 100000*((a.q[1]-gatedy)/(d*d))
...
var A = []
for(let k=0;k<2;k++){
    var max_agent=Math.floor(Math.random()*2)+6
...
function draw(){
    ...
    gateuy=200
    gatedy=300
    drawLine([250,0],[250,gateuy])
    drawLine([250,500],[250,gatedy])
