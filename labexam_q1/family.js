...
function findForce(a){
    ...
    for(let i=0;i<A.length;i++){
        ...
        if(ca.type==a.type){
            ...
            fxrep+=50*((-ca.q[0]+a.q[0])/(d*d))
            fyrep+=50*((-ca.q[1]+a.q[1])/(d*d))
        }
        else{
            fxatt+=0.001*(ca.q[0]-a.q[0])
            fyatt+=0.001*(ca.q[1]-a.q[1])

            fxrep+=1000*((-ca.q[0]+a.q[0])/(d*d))
            fyrep+=1000*((-ca.q[1]+a.q[1])/(d*d))
        }
    }
...
var type = -1
for(let k=0;k<2;k++){
    var max_agent=Math.floor(Math.random()*2)+10
    var fam=Math.floor(Math.random()*max_agent)
    for(let i=0;i<max_agent;i++){
        ...
        if(fam>0){
            ...
            let temp=new Agent(i,type+k*100,[tempx,tempy])
            type-=1
            A.push(temp) 
        }
    }
